...
func TestNewZeroLogJaegerLogger(t *testing.T) {
	assert.NotPanics(t, func() {
		NewZeroLogJaegerLogger(logger.GetLogger("Test Jaeger"))
...
func TestZerologJaegerLogger_Error(t *testing.T) {
	jLogger := NewZeroLogJaegerLogger(logger.GetLogger("Test Jaeger"))
	assert.NotPanics(t, func() {
...
// TestNewZeroLogJaegerLogger tests if the log and trace helper function works
func TestLogAndTraceEmtpyErr(t *testing.T) {
	span := opentracing.StartSpan("NewSpan")
	assert.NotPanics(t, func() {
		LogAndTraceErr(zerolog.Logger{}, span, nil, "ERR")
	}, "Logs without panic")
}
