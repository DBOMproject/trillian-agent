...
func TestLoggerMiddleware(t *testing.T) {
	...
	panicHandler := func(w http.ResponseWriter, req *http.Request) {
		panic(errors.New("expected panic"))
	}

	handler := http.HandlerFunc(helloHandler)
	assert.NotPanics(t, func() { GetLoggerMiddleware(&logger, handler) }, "Logger middleware getter does not panic")
	panHandler := http.HandlerFunc(panicHandler)
	loggerMiddleware := GetLoggerMiddleware(&logger, handler)
	panLoggerMiddleware := GetLoggerMiddleware(&logger, panHandler)

	t.Run("For_OK_Request", func(t *testing.T) {
		mockRequest := httptest.NewRequest("POST", "http://mock-target/", strings.NewReader(""))
		responseRecorder := httptest.NewRecorder()
		loggerMiddleware.ServeHTTP(responseRecorder, mockRequest)
	...
	t.Run("For_Panic", func(t *testing.T) {
		mockRequest := httptest.NewRequest("POST", "http://mock-target/", strings.NewReader(""))
		responseRecorder := httptest.NewRecorder()
		t.Log("\n==== A stack trace being displayed is expected behavior for this test ====\n")
		assert.NotPanics(t, func() {
			panLoggerMiddleware.ServeHTTP(responseRecorder, mockRequest)
