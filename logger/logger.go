...
func GetLoggerMiddleware(logger *zerolog.Logger, next http.Handler) http.Handler {
...
// SetupLoggingMiddleware sets up logging for the REST API
func SetupLoggingMiddleware(next http.Handler) http.Handler {
	zeroLogHTTP := GetLogger("HTTP")
	handler := GetLoggerMiddleware(&zeroLogHTTP, next)
